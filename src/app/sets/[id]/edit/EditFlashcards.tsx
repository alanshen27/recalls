...
export default function EditFlashcards({ setId, initialFlashcards, previewMode, onFlashcardsChange }: EditFlashcardsProps) {
  ...
  useEffect(() => {
      if (previewMode) return; // Skip socket functionality in preview mode
  ...
  useEffect(() => {
    if (previewMode) {
      // In preview mode, just notify parent of changes
    ...
    saveFlashcards(flashcards); // Save current state, not debounced state
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [debouncedFlashcards, previewMode, flashcards, onFlashcardsChange]);
  ...
  const handleFlashcardsUpload = (data: { term: string; definition: string }[]) => {
    setIsSaved(false);
    setFlashcards(prev => [...prev, ...data.map((item, index) => ({ id: `temp_${Date.now()}_${index}`, term: item.term, definition: item.definition }))]);
  };
      ...
      {flashcards.map((flashcard, index) => (
        ...
        <CardContent
          className={`p-4 space-y-4 relative border rounded-md transition-all duration-150
            ${!previewMode && lockedFlashcards.includes(flashcard.id)
      ...
      <div className="flex justify-between">
        ...
        {!previewMode && (
          ...
          <DialogContent className="max-w-2xl">
            ...
            <DialogDescription>
              Upload a CSV file to add flashcards to your set.
            </DialogDescription>
            <FileUpload onUpload={handleFlashcardsUpload} setId={setId} />
          </DialogContent>
        </Dialog>
        )}
        </div>
